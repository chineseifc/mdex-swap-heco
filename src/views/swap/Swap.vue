...
import { isHt } from '@/utils/verify.js';
...
export default {
  ...
  computed: {
    ...
    // 是否HT - WHT
    getWrap() {
      if (isHt(this.fromCur) && this.toCur.address == this.fromCur.address) {
        return true;
      }
      return false;
    },
    // 是否WHT - HT
    getWithdraw() {
      if (isHt(this.toCur) && this.toCur.address == this.fromCur.address) {
        return true;
      }
      return false;
    },
  ...
  methods: {
    ...
    async handleCurBalance() {
      if (this.account) {
        ...
        if (!isHt(this.toCur) && this.toCur.address != '') {
    ...
    async swapETHForExactTokens(amountOut, amountInMax, path, to, deadline) {//主币换pimoon正常
      const jackRouterContract = useJackRouterContract();
      const gas = await jackRouterContract.methods
    ...
    async swapExactTokensForETH(amountIn, amountOutMin, path, to, deadline) {  
      //情况 不正常，有滑点的代币换主币，调用了4.swapExactTokensForETH
      const jackRouterContract = useJackRouterContract();
      const gas = await jackRouterContract.methods
    ...
    async swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline) {  //if 2.usdt换pimoon正常，usdt换JSTS正常，jsts换CBH不正常。
      //调用了两次，一次swapExactTokensForTokensSupportingFeeOnTransferTokens，
      //一次swapExactTokensForTokens
      const jackRouterContract = useJackRouterContract();
      const gas = await jackRouterContract.methods
