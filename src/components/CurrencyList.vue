...
import { HT } from '@/config/constants/ht';
...
export default {
  ...
  data() {
    return {
      ...
      searchErc20: {
        ...
        decimals: '',
        balance: '',
        logoURI: 'https://s1.ax1x.com/2022/11/24/zGP6DU.png',
      },
      status: 0,
      storeCurList: [], //自定义代币
    };
  ...
  async created() {
    ...
    this.currencyList = [HT, ...DEFAULT_TOKEN_LIST.tokens, ...this.storeCurList];
    this.currencyAllList = [HT, ...DEFAULT_TOKEN_LIST.tokens, ...this.storeCurList];
  },
  mounted() {
  ...
  methods: {
    ...
    async handleCurrencyList() {
      ...
      this.currencyList = await this.getHtBalance(tokenList);
    ...
    async getHtBalance(list) {
      const ethObject = HT;
      if (this.showHt) {
        if (this.account) {
          const htBalance = await simpleRpcProvider.eth.getBalance(this.account);
          ethObject.balance = formatAmount(htBalance);
        }
